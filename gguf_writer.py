...
class GGUFWriter:
    ...
    def _pack_val(self, val: Any, vtype: GGUFValueType, add_vtype: bool, sub_type: GGUFValueType | None = None) -> bytes:
        ...
        if pack_fmt is not None:
            # ────── PARCHE: si se espera entero pero viene None, lo convertimos a 0.
            if vtype in (GGUFValueType.UINT32, GGUFValueType.INT32,
                        GGUFValueType.UINT64, GGUFValueType.INT64):
                if val is None:
                    val = 0
                elif not isinstance(val, int):
                    print(f"[DEBUG-GGUF] vtype={vtype.name}, valor={val!r} (tipo={type(val)}) → se esperaba int")
                    val = int(val)
