...
class TextModel(ModelBase):
    ...
    def set_gguf_parameters(self):
        ...
        if (rope_theta := self.hparams.get("rope_theta")) is not None:
            # ─────────────────────────── PARCHE: forzar entero ───────────────────────────
            try:
                int_rope = int(rope_theta)
            except Exception as e:
                raise RuntimeError(f"No se pudo convertir rope_theta={rope_theta!r} a int") from e
            self.gguf_writer.add_rope_freq_base(int_rope)
            logger.info(f"gguf: rope theta = {int_rope}  (convertido de {rope_theta!r})")
            # ──────────────────────────────────────────────────────────────────────────────
...
class LlamaModel(TextModel):
    ...
    def set_gguf_parameters(self):
        """
        Llena todos los campos GGUF esenciales para Mistral-7B, asegurando que
        llama.rope.dimension_count acabe siendo 128 (n_rot=128), como espera LM Studio.
        """

        # 1) Recuperar o forzar hidden_size y num_attention_heads:
        ...
        # 2) Calcular n_rot = hidden_size // num_attention_heads = 4096 // 32 = 128
        rope_dims = n_embd // n_head

        # 3) Leer otros hparams (con optional=True para evitar KeyError si no existen):
        block_count        = self.find_hparam(["num_hidden_layers", "n_layer"], optional=True)
        n_head_kv          = self.find_hparam(["num_key_value_heads", "n_head_kv"], optional=True)
        rms_eps            = self.find_hparam(["rms_norm_eps"], optional=True)
        max_pos_embds      = self.find_hparam(["n_positions", "max_position_embeddings"], optional=True)
        orig_max_pos_embds = self.find_hparam(["original_max_position_embeddings"], optional=True)
        rope_theta         = self.find_hparam(["rope_theta"], optional=True)
        intermediate_size  = self.find_hparam(["intermediate_size"], optional=True)
        sliding_window     = self.hparams.get("sliding_window")

        # 4) CONTEXT LENGTH
        if max_pos_embds is not None:
            self.gguf_writer.add_context_length(int(max_pos_embds))
        else:
            # Mistral-7B suele usar 32768; si no está, lo fijamos en 32768.
            self.gguf_writer.add_context_length(32768)

        # 5) ORIGINAL CONTEXT LENGTH para rope scaling (si existe)
        if orig_max_pos_embds is not None:
            self.gguf_writer.add_rope_scaling_orig_ctx_len(int(orig_max_pos_embds))

        # 6) EMBEDDING LENGTH = hidden_size
        self.gguf_writer.add_embedding_length(n_embd)

        # 7) FEED-FORWARD LENGTH (intermediate_size)
        if intermediate_size is not None:
            self.gguf_writer.add_feed_forward_length(int(intermediate_size))

        # 8) BLOCK (LAYER) COUNT
        if block_count is not None:
            self.gguf_writer.add_block_count(int(block_count))
        ...
        if n_head_kv is not None:
            ...
            # Si no está en hparams, asumimos same as n_head
            self.gguf_writer.add_head_count_kv(n_head)

        # 10) RMSNorm EPSILON (float)
        if rms_eps is not None:
            self.gguf_writer.add_layer_norm_rms_eps(float(rms_eps))
        else:
            # Valor habitual en Mistral-7B
            self.gguf_writer.add_layer_norm_rms_eps(1e-5)

        # 11) ROTARY DIMENSION COUNT (n_rot) — **debe ser 128 para Mistral-7B**
        self.gguf_writer.add_rope_dimension_count(int(rope_dims))

        # 12) ROPE BASE FREQUENCY (rope_theta)
        if rope_theta is not None:
            self.gguf_writer.add_rope_freq_base(int(rope_theta))
        else:
            # Valor por defecto en Mistral-7B
            self.gguf_writer.add_rope_freq_base(1000000)

        # 13) FILE TYPE (UINT32) — Little Endian = 1
        if hasattr(self, "ftype") and self.ftype is not None:
            self.gguf_writer.add_file_type(int(self.ftype))
        else:
            self.gguf_writer.add_file_type(1)

        # 14) VOCAB SIZE
        self.gguf_writer.add_vocab_size(int(self.hparams["vocab_size"]))

        # 15) SLIDING WINDOW (UINT32)
...
class Phi3MiniModel(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        # 1) Recuperar hidden_size y num_attention_heads. Si no existen, forzamos
        #    los valores oficiales de Mistral-7B: hidden_size=4096, num_attention_heads=32.
        n_embd = self.find_hparam(["hidden_size", "n_embd"])
        if n_embd is None:
            n_embd = 4096
        else:
        ...
        # 2) Calcular n_rot (“rope dimension count”) = hidden_size // num_attention_heads = 4096 // 32 = 128.
        rope_dims = n_embd // n_head

        # 3) Leer otros hparams (pueden devolver None si no están definidos):
        block_count        = self.find_hparam(["num_hidden_layers", "n_layer"])
        n_head_kv          = self.find_hparam(["num_key_value_heads", "n_head_kv"])
        rms_eps            = self.find_hparam(["rms_norm_eps"])
        max_pos_embds      = self.find_hparam(["n_positions", "max_position_embeddings"])
        orig_max_pos_embds = self.find_hparam(["original_max_position_embeddings"])
        rope_theta         = self.find_hparam(["rope_theta"])
        intermediate_size  = self.find_hparam(["intermediate_size"])
        sliding_window     = self.hparams.get("sliding_window")
        ...
        if max_pos_embds is not None:
            ...
            # Mistral-7B suele usar 32768, pero si no viene en hparams, ponemos 32768.
            self.gguf_writer.add_context_length(32768)
        ...
        if n_head_kv is not None:
            ...
            # Si no está en hparams, asumimos que key/value heads = n_head para Mistral-7B.
            self.gguf_writer.add_head_count_kv(n_head)
        ...
        if rms_eps is not None:
            ...
            # Valor por defecto de Mistral-7B es 1e-5
            self.gguf_writer.add_layer_norm_rms_eps(1e-5)

        # 11) ROTARY DIMENSION COUNT (n_rot) – OBLIGATORIO que sea 128 para Mistral-7B
        self.gguf_writer.add_rope_dimension_count(int(rope_dims))
        ...
        if rope_theta is not None:
            ...
            # Mistral-7B usa 1 000 000 como valor por defecto
            self.gguf_writer.add_rope_freq_base(1000000)
